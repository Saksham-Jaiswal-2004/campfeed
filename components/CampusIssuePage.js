    ...
    <div className="w-full h-screen overflow-y-auto flex justify-center">
      <div className="w-full max-w-6xl px-6 py-8">
        <div className="flex items-center gap-3 text-xs text-gray-400 mb-4">
          ...
          <IoIosArrowForward />
          <span className="text-white/90 truncate max-w-[40ch]">
            {issue.title || "Issue"}
          </span>
        </div>
            ...
            <div className="absolute left-6 bottom-6 right-6 text-white flex items-end justify-between gap-4">
              <div className="max-w-3xl">
                <div className="flex flex-wrap gap-2 mb-3">
                  <span className="px-3 py-1 rounded-full text-xs bg-white/10 backdrop-blur border border-white/10">
                    {categoryLabels[issue.category_id] || "Issue"}
                  </span>
                  <span
                    className={`px-3 py-1 rounded-full text-xs ${issue.status === "resolved" ? "bg-green-500/15 text-green-300" : issue.status === "rejected" ? "bg-red-500/15 text-red-300" : issue.status === "in_progress" ? "bg-yellow-500/15 text-yellow-300" : "bg-gray-500/15 text-gray-200"}`}
                  >
                ...
                <p className="mt-2 text-sm sm:text-base text-white/75 max-w-2xl line-clamp-3">
                  {issue.description}
                </p>
              </div>
            </div>
                ...
                <div className="flex items-center justify-between gap-3 mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    {issue.priority === "critical" && (
                      <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs bg-red-500/15 text-red-300 border border-red-500/20">
                        <CgDanger /> Critical
                      </span>
                    )}
                    {issue.priority === "high" && (
                      <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs bg-orange-500/15 text-orange-300 border border-orange-500/20">
                        <CgDanger /> High
                      </span>
                    )}
                    {issue.priority === "medium" && (
                      <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs bg-yellow-500/15 text-yellow-300 border border-yellow-500/20">
                        <MdOutlineInfo /> Medium
                      </span>
                    )}
                    {issue.priority === "low" && (
                      <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs bg-green-500/15 text-green-300 border border-green-500/20">
                        <FiCheckCircle /> Low
                      </span>
                    )}
                  </div>
              ...
              <div className="rounded-xl border border-gray-800 bg-[#031025] p-5">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-xs text-gray-500">
                    {issueImages.length} attachment
                    {issueImages.length === 1 ? "" : "s"}
                  </span>
                </div>
              ...
              <div className="rounded-xl border border-gray-800 bg-[#031025] p-5 space-y-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-200 mt-2">
                      ...
                      {new Date(issue.created_at._seconds * 1000).toLocaleString("en-GB", {
                          weekday: "short",
                          day: "2-digit",
                          month: "short",
                          year: "numeric",
              ...
              <div className="rounded-xl border border-gray-800 bg-[#031025] p-5 space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {(issue.tags || []).map((tag, index) => (
                    <span
                      key={index}
                      className="px-3 py-1 rounded-full text-xs bg-cyan-500/10 text-cyan-200 border border-cyan-500/20"
                    >
                      {tag}
                    </span>
                  ))}
